...
def _ask_batch(client, query, project, batch_pages, processed_folder, start_index,
               batch_num=None, total_batches=None, scope_context=None, library_context=None):
...
def _synthesize(client, query, project, batch_answers, scope_context=None, library_context=None):
    ...
    scope_prefix = f"Project work scope: {scope_context}\n\n" if scope_context else ""
    prompt = (
        f"{library_prefix}{scope_prefix}You asked multiple batches of engineering drawings from the project "
...
def search_drawings(query, project_id, api_key, processed_folder, doc_type=None, scope_context=None, library_context=None):
    ...
    if total_batches == 1:
        answer, index_map = _ask_batch(
            ...
            scope_context=scope_context, library_context=library_context,
        )
        if not index_map:
    ...
    final_answer = _synthesize(client, query, project, batch_answers, scope_context=scope_context, library_context=library_context)
    return {
        "answer": final_answer,
